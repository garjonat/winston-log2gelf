...
const sendHTTPGelf = function (host, port, tls, user, pass) {
	...
	var username = user;
	var password = pass;
	var authorizationBasic = new Buffer(username + ":" + password).toString("base64");
    let clientType;
    ...
    return (msg) => {
        ...
        const req = clientType.request(options, (res) => {
            // usefull for debug
			if (res.statusCode==401) console.log('Error autentificacio servidor Graylog, revisar user/password: ', res.statusCode);
        });
